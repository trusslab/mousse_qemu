...
static void QEMU_NORETURN force_sig(int target_sig)
{
    ...
    if (core_dumped) {
        ...
        (void) fprintf(stderr, "%s [1]: qemu: uncaught target signal %d (%s) - %s\n",
            __FUNCTION__,target_sig, strsignal(host_sig), "core dumped" );
    }
...
int queue_signal(CPUArchState *env, int sig, target_siginfo_t *info)
{
    ...
    fprintf(stderr, "%s [1]: queue_signal: sig=%d\n",__FUNCTION__,
            sig);
#endif
    k = &ts->sigtab[sig - 1];
    queue = gdb_queuesig ();
...
struct target_ucontext_v2 {
    ...
    struct target_sigcontext tuc_mcontext;
    target_sigset_t  tuc_sigmask;	/* mask last for extensibility */
    char _unused[128 - sizeof(target_sigset_t)];

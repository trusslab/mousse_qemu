...
unsigned long mq_open(const char *name, int oflag, mode_t mode, struct mq_attr *attr); 
unsigned long mq_open(const char *name, int oflag, mode_t mode, struct mq_attr *attr) 
{
    printf("[-] %s(%d)-%s\n",__FILE__,__LINE__,__FUNCTION__);
    assert(0);
...
ssize_t readahead (int __fd, unsigned long __offset, size_t __count);
ssize_t readahead (int __fd, unsigned long  __offset, size_t __count)
{
    printf("[-] %s(%d)-%s\n",__FILE__,__LINE__,__FUNCTION__);
    assert(0);
                                               ...
                                               struct ipc_perm *host_ip)
{
    ...
    target_ip = &(target_sd->sem_perm);
#ifdef HOST_ARM
    target_ip->__key = tswapal(host_ip->key);
#else
...
#include "ioctls.h"
    {0xc1007c00, 0xc1007c00, "MEDIA_IOC_DEVICE_INFO", IOC_R, 0, { MK_PTR(MK_STRUCT(STRUCT_media_device_info))}},
    {0xc1007c01, 0xc1007c01, "MEDIA_IOC_ENUM_ENTITIES", IOC_R | IOC_W, 0, { MK_PTR(MK_STRUCT(STRUCT_media_entity_desc))}},
    {0xc0046209, 0xc0046209, "BINDER_VERSION", IOC_R | IOC_W, 0, { MK_PTR(MK_STRUCT(STRUCT_binder_version))}},
    {0x40046205, 0x40046205, "BINDER_SET_MAX_THREADS", IOC_W, 0, {MK_PTR(TYPE_ULONG)}}, /* size_t */
    {0xc0306201, 0xc0306201, "BINDER_WRITE_READ", IOC_R | IOC_W, 0, {MK_PTR(MK_STRUCT(STRUCT_binder_write_read))}}, 
    { 0, 0, },
};
...
static abi_long do_ioctl(int fd, abi_long cmd, abi_long arg)
{
    ...
    int target_size;
    void *argptr;
    ret = get_errno(ioctl(fd, cmd, arg));
    return ret;
    ie = ioctl_entries;
    for(;;) {
        if (ie->target_cmd == 0) {

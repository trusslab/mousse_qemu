...
/* Added to support user mode qemu with kvm */
#ifdef CONFIG_USER_KVM
#include "kvm.h"
//copied from vl.c
#define DEFAULT_RAM_SIZE 256
/* init the memory */
ram_addr_t ram_size = DEFAULT_RAM_SIZE * 1024 * 1024;;
int smp_cpus = 1;
int smp_cores = 1;
int smp_threads = 1;
...
#else
	int ret1 = kvm_init(); 
	/* init page in both qemu and s2e */
	page_init();
	if(ret1 < 0) {
...
/* Passing all the register values to s2e VCPU */
#ifdef CONFIG_USER_KVM
	cpu_synchronize_all_post_init();
	user_kvm_cpu_exec(env);
#else
       cpu_loop(env);
#endif
   return 0;
